...
def poligono_e_valido(vertices: List[CelulaNaGrade]) -> Tuple[bool, str]:
    ...
    if checar_vertices_alinhados(vertices_limpos):
        return False, "Polígono degenerado (reta)"
...
def main():
    ...
    while executando:
                    ...
                    if botao["rect"].collidepoint(evento.pos):
                        ...
                        # Cores pré-definidas
                            ...
                            if len(vertices_distintos) >= 3:
                                ...
                                print(f"Apenas {len(vertices_distintos)} vértices distintos. Use 'Limpar Tudo' para resetar.")
                        elif acao == "remover":
                            estado.remover_ultimo_vertice()

                # Escolheu algum vértice
                if evento.button == 1:
        ...
        if estado.vertices:
            ...
            # Desenha linha temporária se estiver dentro dos limites do tabuleiro
