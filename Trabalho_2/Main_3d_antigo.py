...
from Aux_3D_antigo import (Estado3D, init_opengl, draw_sphere, update_light, render_3d_to_texture, FBO,
...
def create_buttons():
    ...
    buttons.extend([
        ...
        {"texto": "Ortográfica", "rect": pygame.Rect(margin + btn_w_light + spacing, y_start, btn_w_light, btn_h), "acao": "ortografica", "tipo": "proj"},
    ])
    ...
    buttons.extend([
        ...
        {"texto": "Rotação", "rect": pygame.Rect(margin + btn_w_light + spacing, y_start, btn_w_light, btn_h), "acao": "rotacao", "tipo": "tipo_tg"},
        {"texto": "Escala", "rect": pygame.Rect(margin + 2 * (btn_w_light + spacing), y_start, btn_w_light, btn_h), "acao": "escala", "tipo": "tipo_tg"},
    ])
    ...
    buttons.extend([
        ...
        {"texto": "Y", "rect": pygame.Rect(margin + btn_w_light + spacing, y_start, btn_w_light, btn_h), "acao": "eixo_y", "tipo": "eixo_tg"},
        {"texto": "Z", "rect": pygame.Rect(margin + 2 * (btn_w_light + spacing), y_start, btn_w_light, btn_h), "acao": "eixo_z", "tipo": "eixo_tg"},
        {"texto": "+", "rect": pygame.Rect(margin + 4 * (btn_w_light + spacing), y_start, btn_w_light, btn_h), "acao": "mais", "tipo": "direcao_tg"},
        {"texto": "-", "rect": pygame.Rect(margin + 5 * (btn_w_light + spacing), y_start, btn_w_light, btn_h), "acao": "menos", "tipo": "direcao_tg"},
    ])
    ...
    buttons.extend([
        ...
        {"texto": "Resetar", "rect": pygame.Rect(margin + btn_w_light + spacing, y_start, btn_w_light, btn_h), "acao": "reseta", "tipo": "transformacao"},
    ])
...
def desenhar_botoes(gui_surface, font, font_small, font_tiny, estado_atual):
    #estado.lighting_model, estado.projecao, , estado.eixo, estado.direcao
    cor_atual = estado_atual.material
    shape_atual = estado_atual.shape
    camera_atual = estado_atual.camera_angle
    lighting_atual = estado_atual.lighting_model
    proj_atual = estado_atual.projecao
    tg_atual = estado_atual.tipo
    eixo_atual = estado_atual.eixo
    dir_atual = estado_atual.direcao
    ...
    for botao in BOTOES_3D:
        if "cor" in botao:
            ...
            is_selected = (
                ...
                (botao.get("tipo") == "lighting" and botao["acao"] == lighting_atual) or
                (botao.get("tipo") == "proj" and botao["acao"] == proj_atual) or
                (botao.get("tipo") == "tipo_tg" and botao["acao"] == tg_atual) or
                (botao.get("tipo") == "eixo_tg" and botao["acao"] == eixo_atual) or
                (botao.get("tipo") == "direcao_tg" and botao["acao"] == dir_atual)
            )
...
def desenhar_hud(gui_surface, font_small, font_tiny, estado):
    ...
    col_width = width // 3
    info_items = [
        ("Translação", estado.posicao),
        ("Rotação", estado.rotacao),
        ...
        try:
            while True:  # Non-blocking poll
                ...
                if cmd['type'] == 'light_pos':
                    ...
                    estado.projecao = cmd['value']
                elif cmd['type'] == 'tipo_tg':
                    estado.tipo = cmd['value']
                elif cmd['type'] == 'eixo_tg':
                    estado.eixo = cmd['value']
                elif cmd['type'] == 'direcao_tg':
                    estado.direcao = cmd['value']
                elif cmd['type'] == 'transformacao':
                    if cmd['value'] == 'aplica':
...
def run_gui_window(cmd_queue, state_queue=None):
    ...
    while running:
                ...
                if event.button == 1:
                    for botao in BOTOES_3D:
                        if botao["rect"].collidepoint(event.pos):
                            ...
                            elif acao in ["perspectiva", "ortografica"]:
                                estado.projecao = acao
                                cmd_queue.put({'type': 'proj', 'value': acao})
                            elif acao in ["translacao", "rotacao", "escala"]:
                                estado.tipo = acao
                                cmd_queue.put({'type': 'tipo_tg', 'value': acao})
                            elif acao in ["eixo_x", "eixo_y", "eixo_z"]:
                                estado.eixo = acao
                                cmd_queue.put({'type': 'eixo_tg', 'value': acao})
                            elif acao in ["mais", "menos"]:
                                estado.direcao = acao
        ...
        # Draw GUI controls
        desenhar_botoes(screen_gui, font, font_small, font_tiny, estado)
